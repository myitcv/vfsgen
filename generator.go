...
func Generate(input http.FileSystem, opt Options) error {
	...
	var toc toc
	err = findAndWriteFiles(buf, input, &toc, !opt.DontWriteModTime)
	if err != nil {
		return err
	}
...
func findAndWriteFiles(buf *bytes.Buffer, fs http.FileSystem, toc *toc, writeModTime bool) error {
